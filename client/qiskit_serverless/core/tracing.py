...
def get_tracer(
    ...
    If agent host and ports are provided then will include OTLP exporter as provider.

    Args:
        instrumenting_module_name: module name for tracing
        agent_host: OTLP collector host
        agent_port: OTLP collector port

    Returns:
        tracer
    """
    resource = Resource(attributes={SERVICE_NAME: f"qs.{os.environ.get(QS_OT_PROGRAM_NAME, OT_PROGRAM_NAME_DEFAULT)}"})
