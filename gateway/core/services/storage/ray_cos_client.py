...
class RayCOSClient(AbstractCOSClient):
    """COS client backed by a single bucket used for all Ray job storage."""
    ...
    def _boto3_client(self):
        if self._client is None:
            self._client = boto3.client(
                "s3",
                endpoint_url=settings.RAY_COS_ENDPOINT,
                aws_access_key_id=settings.RAY_COS_ACCESS_KEY,
                aws_secret_access_key=settings.RAY_COS_SECRET_KEY,
            )
        return self._client
    ...
    def put_object_bytes(self, key: str, content: bytes, working_dir: WorkingDir = WorkingDir.USER_STORAGE) -> None:
        try:
            self._boto3_client.put_object(Bucket=settings.RAY_COS_BUCKET, Key=key, Body=content)
        except ClientError as e:
    ...
    def delete_object(self, key: str, working_dir: WorkingDir = WorkingDir.USER_STORAGE) -> bool:
        try:
            self._boto3_client.delete_object(Bucket=settings.RAY_COS_BUCKET, Key=key)
    ...
    def list_objects(self, prefix: str, working_dir: WorkingDir = WorkingDir.USER_STORAGE) -> list[str]:
        try:
            response = self._boto3_client.list_objects_v2(Bucket=settings.RAY_COS_BUCKET, Prefix=prefix)
