...
class KafkaEventStreamsClient(EventStreamsClient):
    ...
    def _emit_job_completed(self, job: Job, ended_at: datetime, metric_type: str | None = None) -> None:
        ...
        logger.info(
            "job_id=%s Emitting job_completed event metric_type=%s metric_value=%s",
            job.id,
            metric_type,
            usage_seconds,
        )
    ...
    def _emit_license_fee(self, job: Job) -> None:
        ...
        parts = [LICENSE_FEE_METRIC_TYPE, job.program.provider.name, job.program.title, job.function_size.function_size]
