...
class TestLogsStorage:
    ...
    def test_get_public_logs_returns_none_if_not_found(self, tmp_path, settings):
    ...
    def test_get_private_logs_returns_none_if_not_found(self, tmp_path, settings):
        """get_public_logs returns None if file doesn't exist."""
        settings.MEDIA_ROOT = str(tmp_path)
    ...
    def test_save_and_get_public_logs_user_job(self, tmp_path, settings):
        """Test saving and retrieving public logs for a user job."""
        settings.MEDIA_ROOT = str(tmp_path)
        job = self._create_job("auth1", provider=None, job_id="job-123")
        expected_path = os.path.join(str(tmp_path), "auth1", "logs", "job-123.log")
        storage = LogsStorage(job)
