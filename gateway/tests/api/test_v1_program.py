...
class TestProgramApi:
    ...
    def test_run(self, settings):
        ...
        expected_arguments_path = os.path.join(settings.MEDIA_ROOT, user.username, "arguments")
    ...
    def test_provider_run(self, settings):
        """Tests run existing authorized."""
        user = models.User.objects.get(username="test_user_2")
        self.client.force_authenticate(user=user)
    ...
    def test_active_jobs_queue_limit(self, settings):
        """Tests queue limit."""
        settings.LIMITS_ACTIVE_JOBS_PER_USER = self.LIMITS_ACTIVE_JOBS_PER_USER
        ...
        assert (
            self.LIMITS_ACTIVE_JOBS_PER_USER == Job.objects.filter(author=user, status__in=Job.ACTIVE_STATUSES).count()
        )
    ...
    def test_run_user_function_with_same_title_as_provider_function(self, settings):
        """
