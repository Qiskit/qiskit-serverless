...
def files_delete(request: Request) -> Response:
    ...
    logger.info(
        "[files-delete] user_id=%s function=%s provider=%s file=%s accessible_functions=%s",
        user.id,
        function,
        provider,
        file,
    ...
    FilesDeleteUseCase().execute(user, provider, function, file, accessible_functions=accessible_functions)
    logger.info(
