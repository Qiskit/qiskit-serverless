...
class InputSerializer(serializers.Serializer):
    ...
    class Meta:
        ...
        ref_name = "FilesProviderDownloadInputSerializer"
...
@endpoint("files/provider/download", method="GET")
...
def files_provider_download(request: Request) -> StreamingHttpResponse:
    ...
    logger.info(
        "[files-provider-download] user_id=%s function=%s provider=%s file=%s accessible_functions=%s",
        user.id,
        function,
        provider,
        file,
    ...
    result: Tuple[Iterator[bytes], str, int] = FilesProviderDownloadUseCase().execute(
