...
class ProgramViewSet(viewsets.ModelViewSet):
    ...
    def run_program(self, request):
        ...
        if serializer.is_valid():
            ...
            existing_programs = Program.objects.filter(author=request.user, title__exact=program.title)
            if existing_programs.count() > 0:
                # take existing one
                program = existing_programs.first()
            program.artifact = request.FILES.get("artifact")
            # TODO: check format of file or create serializer
            ...
            job = Job(program=program, author=request.user, ray_job_id=ray_job_id, compute_resource=compute_resource)
            job.save()
