...
| application.externalDatabaseCredentials.enabled | enable / disable creation of the Postgres binding as an `ExternalSecret` (ignored when `platform: kind`, which always creates a plain-text `Secret` instead)                            |
| secrets.servicePsql.*                     | `name` and `key` let you to specify the secret; `value` the value for the secret (`platform: kind` only); `secretRef` and `secretStoreName` point to the entry in the external secret store |
